...
(function($) {
  ...
  function chunkedExpand(array, process){
    setTimeout(function(){
        var item = array.shift();
        $(item).expand();

        if (array.length > 0){
            expandTimout = setTimeout(arguments.callee, 10);
        }
    }, 10);
  }
      ...
      if(node.hasClass("parent")) {
        ...
        if(options.expandable) {
          ...
          if(node.attr('id').length == 10){
		  	cell.children('span').children('.expandAllFromHere').click(function() {node.toggleAllDescendants(); return false;}).mousedown(function() { return false; });
          	cell.children('span').children('.expandAllFromHere').keydown(function(e) { if(e.keyCode == 13) { node.toggleAllDescendants(); return false; }});
          }else{
          	cell.children('span').children('.expandAllFromHere').addClass('hidden');
          }
          $(cell[0]).click(function() { node.toggleBranch(); return false; });
