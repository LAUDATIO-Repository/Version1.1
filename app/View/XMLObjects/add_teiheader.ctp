...
$this->set('navbarArray',array(array('Modify','modify'),array('Modify Object','modifyObject/'.urlencode($id)),array('Add TEI-header')));
...
if(isset($ingestedObject)){
    echo $this->html->link('View new version of TEI-Header',array('controller' => 'XMLObjects', 'action' => 'corpus/'.$ingestedObject));
    echo '</br>';
    echo '<p></p>';
    echo '</br>';
}
...
<form id="ingestform" method="POST" onsubmit="return validateForm()" enctype="multipart/form-data" xmlns="http://www.w3.org/1999/html"
    ...
    <table id="uploadform" border="0" cellpadding="0" cellspacing="4">
     ...
     <tr>
        ...
        <td>Label: <input id="label" name="label" type="text" size="15" maxlength="30"></td>
         <td>Scheme: <select id="scheme" name="scheme">
             <?php
                foreach($schemes as $scheme){
                    if($scheme == $activeScheme){
                        echo '<option selected>'.$scheme.'</option>';
                    }else{
                        echo '<option>'.$scheme.'</option>';
                    }
    ...
    <br>
    <input id="formSubmitButton" type="submit" value="Update TEI-header!">
</form>
