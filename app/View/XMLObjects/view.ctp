...
<?php
	$this->set('navbarArray',array(array('Browse','view')));
	echo '<table>';
	
	echo '<th>Corpora<span class="helptooltip"><p>Click on an object to display the corpus.</p>';
	...
	//$xml = simplexml_load_string(html_entity_decode($xmloutput)) or die("Error: Can not create object");
		...
		foreach ($corpora as $corpus) {
                ...
                #echo '<td><button type="button" onclick="location.href=\''.Router::url(array('controller' => 'XMLObjects','action' => 'corpus',$pid)).'\'" value="Browse Corpus">Browse Corpus</button></td>';
            echo '</tr>';
		}
