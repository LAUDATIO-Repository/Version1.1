...
<script>
    ...
    /*
     ...
     row = new Range(found.start.row+1, found.start.column-1, found.end.row+1, found.end.column); //Find Coordinates, after Line "start_#D1_SavePos"
     var D1SavePos = (editor.session.getTextRange(row)) ; //read Text in row
     // rewrite
     var newD1SavePos = document.getElementById("highestBTTXT").value;  //get value from input field
     D1SavePos = D1SavePos.replace(/R13=0/, "R13=" + newD1SavePos); //re write R-Parameter with inputvalue
     // write in editor
     editor.selection.moveCursorToPosition({row: row, column: 0}) ;  //mark line
     editor.session.replace(new Range(row, 0, row, Number.MAX_VALUE), D1SavePos); //write
     */
