...
class XMLObject extends AppModel {
    ...
    public function getCreator($corpus_name){
        ...
        if($corpus_creator !== false){
            if(isset($corpus_creator['User']['username'])) {
    ...
    public function handle($pid){
        ...
        $response = XMLObject::registerHandlePID($pid,$version);
        if($response['http_code'] != 201){
            return $response;
        }
        return XMLObject::appendHandlePID($response['response'],$pid,$version);
    }
    ...
    public function registerHandlePID($pid,$version = null){
        ...
        $handleAdress = $confHandle['handle_address'];
        $handle_userpwd = $confHandle['handle_userpwd'];

        if($version==null){
            $version = XMLObject::getFirstTeiVersion($pid);
        }
        $objectURL = Router::url(array('controller' => 'XMLObjects','action' => 'corpus',$pid,$version), true);
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_HEADER, 1);
        curl_setopt($ch, CURLOPT_URL, $handleAdress );
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
        curl_setopt($ch, CURLOPT_USERPWD, $handle_userpwd);
        curl_setopt($ch, CURLOPT_VERBOSE, 1);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_POST, true);
        ...
        if($result_array['http_code'] != 201){
            $this->log('error','Handle PID not registered: '.$response);
            $result_array['response'] = $response;
            return $result_array;
        }
        ...
        $result_array['response'] = $response2;
        return $result_array;
    }
    ...
    public function updateHandle($pid){
        ...
        foreach($pids as $header => $handle){
            ...
            $handleAddress= $handleAdress.$handle;//"http://hdl.handle.net/11022/$handle?no-redirect";
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_HEADER, 1);
    ...
    public function unregisterHandlePID($handlePID){
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_HEADER, 1);
        curl_setopt($ch, CURLOPT_URL, $handleAdress.$handlePID );
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
        curl_setopt($ch, CURLOPT_USERPWD, $handle_userpwd);
        curl_setopt($ch, CURLOPT_VERBOSE, 1);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    ...
    public function saveLicense($post){
        ...
        if(isset($post['cc_js_want_cc_license'])){
            $license_array['cc_js_want_cc_license'] = $post['sure'];
        }
    ...
    public function setLicense($pid){
        $confFedora = Configure::read('FedoraConfig');
        $fedoraAdress = $confFedora['fedora_address_port'];

        $license_array = array();
        if(isset($_POST['cc_js_want_cc_license'])){
            $license_array['cc_js_want_cc_license'] = $_POST['sure'];
        }
        if(isset($_POST['cc_js_result_uri'])){
            $license_array['cc_js_result_uri'] = $_POST['cc_js_result_uri'];
        }
        if(isset($_POST['cc_js_result_img'])){
            $license_array['cc_js_result_img'] = $_POST['cc_js_result_img'];
        }
        if(isset($_POST['cc_js_result_name'])){
            $license_array['cc_js_result_name'] = $_POST['cc_js_result_name'];
        }
        ...
        XMLObject::addDatastream($pid,$file,urlencode("license"),urlencode("Creative Commons License"));
        unlink($file);
    ...
    public function validateModifyObject($post,$validate_array){
        ...
        if($validate_array['license']!=$post['license']) $change = true;

        if($validate_array['cc_js_want_cc_license']!=$post['cc_js_want_cc_license']) $change = true;
