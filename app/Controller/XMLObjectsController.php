...
class XMLObjectsController extends AppController {
    ...
    public function corpus($pid = null,$version = null){
        ...
        if (!isset($xmlObject->teiHeader)) {
            ...
            $date_array = array();
            foreach ($xmlObject->teiHeader->revisionDesc->change as $change) {
                array_push($date_array, $change['n']);
            }
    ...
    // 030615--------------
    public function replaceUrl($regText) {
        $reg_exUrl = '%(?:(?:https?|ftp)://)(?:\S+(?::\S*)?@|\d{1,3}(?:\.\d{1,3}){3}|(?:(?:[a-z\d\x{00a1}-\x{ffff}]+-?)*[a-z\d\x{00a1}-\x{ffff}]+)(?:\.(?:[a-z\d\x{00a1}-\x{ffff}]+-?)*[a-z\d\x{00a1}-\x{ffff}]+)*(?:\.[a-z\x{00a1}-\x{ffff}]{2,6}))(?::\d+)?(?:[^\s]*)?%iu';

        if(preg_match($reg_exUrl, $regText, $url)) {
            //$url[0] = str_replace('Not set', "", $url[0]);
            return preg_replace($reg_exUrl, '<a href="'.$url[0].'"rel="nofollow">'.$url[0].'</a>', $regText);
        }
        return $regText;
    }
...
//            Cache::write(urlencode($pid.$version),$corpus, 'short'); //TODO: handle error if cache full or something
    ...
    public function modifyObject($pid = null, $dsid = null){
        $successmsg = "";
        ...
        #check id
        if($pid) {
            $pid = urldecode($pid); // laudatio:test
        }
        else {
        ...
        if(isset($_POST) && !empty($_POST)) {
            ...
            try {
                ...
                $validate_array['license']=$xmlObject->objLicense;
                ...
                if($flashmsg != '') {
                    ...
                    if($validate_array['license']!=$_POST['license']) $data['license']=$_POST['license'];

                    if($validate_array['cc_js_want_cc_license']!=$_POST['cc_js_want_cc_license']) $data['cc_js_want_cc_license']=$_POST['cc_js_want_cc_license'];
                    ...
                    if($result_array['http_code'] == '200') {
                        ...
                        if($this->XMLObject->doIndexing($pid) == 1) {
                            $return_array = $this->XMLObject->indexLatestVersion($pid);

                            if($return_array['http_code']== 201) {
                                $successmsg .= "</br>Corpus indexed.";
                                $this->Session->setFlash($successmsg, 'default', array('class' => 'success'));
                            }
                            else if ($return_array['http_code'] == 400) {
                                $errormsg = $return_array['response'];
                                $this->Session->setFlash('Corpus not indexed.'.$errormsg, 'default');
                            }
                        }
                        $this->set('ingestedObject',urlencode($pid));
                    }
                    else {
        ...
        if(!empty($_POST)){
            $this->set('license',$this->XMLObject->setLicense($pid));
        }
    ...
    public function import() {
        if(!empty($_POST)) {
            ...
            if($errormsg != ''){
                ...
                if(!$error) {
                    if(isset($_FILES['teifile']) && isset($_POST['openAccess'])) {
                        $sXmlFile = '/tmp/'.basename($_FILES['teifile']['tmp_name']);
                        $return_array = $this->XMLObject->handle($_POST['id']);
                        $this->XMLObject->saveLicense($_POST);
                        if($return_array['http_code']== 201) {
                            $successmsg .= "</br>PID assigned.";
                        }
                        else {
                            $errormsg .= "</br>PID not indexed. Error:".$return_array['response'];
                            $error = true;
                        }
                    }
                    ...
                    if(isset($_FILES['teifile']) && isset($_POST['doIndex'])) {
                        ...
                        if($return_array['http_code']== 201) {
                            ...
                            $findMe   = ' nested:';
                            $pos = strstr($return_array['response'], $findMe);
                            $errormsg .= "Corpus not indexed.</br></br>Error:".$pos;
                            $error = true;

                            $this->Session->setFlash($errormsg, 'default');
                        };
                    }
                }
                if($error) {
    ...
    public function addTeiheader($id) {
        if(isset($_POST) && !empty($_POST) && isset($_POST['label']) && !empty($_POST['label'])) {
            ...
            if(isset($_FILES['teifile'])) {
                $sXmlFile = '/tmp/'.basename($_FILES['teifile']['tmp_name']);
                $json = $this->XMLObject->javaXSLT($sXmlFile);
                $return_array = $this->XMLObject->indexJson($json,$id);
                ...
                if($return_array['http_code'] == 200) {
                    ...
                    $findMe   = ' nested:';
                    $pos = strstr($return_array['response'], $findMe);
                    $errormsg .= "New TEI-Header not indexed.</br></br>Error:".$pos;
                    $error = true;
            ...
            if($error) {
                ...
                $this->Session->setFlash('Corpus imported. '.$successmsg, 'default', array('class' => 'success'));
                $this->set('ingestedObject',urlencode($id));
            }
        }
    ...
    public function downloadTei($pid) {
        ...
        $path = $fedoraAdress.'/fedora/objects/'.$pid.'/datastreams/TEI_header/content';
    ...
    public function downloadDatastream($pid,$dsid,$asOfDateTime = null){
        ...
        $fedoraAdress_port = $confFedora['fedora_address_port'];
        ...
        if($asOfDateTime) {
            $url[] = $fedoraAdress_port."/fedora/objects/".$pid."/datastreams/".$dsid."/content?asOfDateTime=$asOfDateTime";
        }else{
            $url[] = $fedoraAdress_port."/fedora/objects/".$pid."/datastreams/".$dsid."/content";
        }
            ...
            if(isset($_FILES) && $_FILES['teifile']['size'] != 0) {
                ...
                if($valid) {
                    ...
                    if($response['http_code'] == 201){
                        ...
                        if($this->XMLObject->doIndexing($pid))
                            $this->XMLObject->indexLatestVersion($pid);
                        $return_array = $this->XMLObject->handle($pid);
                        if($return_array['http_code']!= 200) {
                            $error = "</br>Could not create handle pid. Error:".$return_array['response'];
                        }
                    }
                ...
                if(isset($_FILES['teifile']) && isset($_POST['doIndex'])) {
                    $sXmlFile = '/tmp/'.basename($_FILES['teifile']['tmp_name']);
                    $json = $this->XMLObject->javaXSLT($sXmlFile);
                    $this->XMLObject->saveLicense($_POST); //new
                    $return_array = $this->XMLObject->indexJson($json,$pid);

                    #before: response['http_code] == 201 -> wrong
                    if($return_array['http_code'] == 200) {
                        $successmsg .= "</br>Corpus indexed.";
                        ...
                        $findMe   = ' nested:';
                        $pos = strstr($return_array['response'], $findMe);
                        $errormsg .= "</br></br>Error:".$pos;
                        $error = true;

                        $this->Session->setFlash('New version not indexed.'.$errormsg, 'default');
                    };
                }
                //}
            }
    ...
    public function dsVersionHistory($pid, $dsid) {
        if(isset($_POST) && !empty($_POST)) {
            if(strpos($_POST['dsID'],'TEI-header') !== 0) {
    ...
    private function modifyPermission($corpus_id) {
        ...
        $user_name = $this->Session->read('Auth.User.username');
        $user_group = $this->Session->read('Auth.User.Group.id');
        if($this->XMLObject->isCorpusCreator($user_id,$user_name,$corpus_id) || $this->XMLObject->isCorpusUser($user_id,$user_name,$corpus_id) || $this->XMLObject->isCorpusGroup($user_group,$corpus_id)) {
            return true;
        }
        else {
            return false;
